...
import { CHAT_TOOLS } from "../ai/tools-definitions.js";
...
function buildInitialMessages(history: { role: string; content: string }[], newMessage: string): Message[] {
...
function streamLine(encoder: InstanceType<typeof TextEncoder>, controller: ReadableStreamDefaultController<Uint8Array>, obj: Record<string, unknown>): void {
...
export function chatRouter(): Router {
  ...
  router.post("/", async (req: AuthRequest, res: Response) => {
    ...
    let body: { message: string; history: { role: string; content: string }[]; conversationId?: string | null; createNew?: boolean; stream?: boolean };
    try {
      body = parseBody(chatBodySchema, req);
    } catch {
    ...
    if (!conversationId) {
      if (createNew) {
        const titre = message.slice(0, 45) + (message.length > 45 ? "…" : "") || "Nouvelle conversation";
        const { data: created, error: insertErr } = await supabase.from("ai_conversations").insert({ utilisateur_id: user.id, titre }).select("id").single();
        if (!insertErr && created?.id) conversationId = created.id;
      } else {
        const { data: existing } = await supabase.from("ai_conversations").select("id").eq("utilisateur_id", user.id).order("date_mise_a_jour", { ascending: false }).limit(1).maybeSingle();
        if (existing?.id) conversationId = existing.id;
        else {
          const { data: created, error: insertErr } = await supabase.from("ai_conversations").insert({ utilisateur_id: user.id, titre: "Nouvelle conversation" }).select("id").single();
          if (!insertErr && created?.id) conversationId = created.id;
        }
      }
    }
    ...
    if (conversationId) {
      await supabase.from("ai_messages").insert({ conversation_id: conversationId, role: "user", contenu: message });
      await supabase.from("ai_conversations").update({ date_mise_a_jour: new Date().toISOString() }).eq("id", conversationId).eq("utilisateur_id", user.id);
    }
    ...
    if (!useStream) {
      ...
      if (conversationId) {
        await supabase.from("ai_messages").insert({ conversation_id: conversationId, role: "assistant", contenu: reply });
        await supabase.from("ai_conversations").update({ date_mise_a_jour: new Date().toISOString() }).eq("id", conversationId);
      }
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        try {
          ...
          const hasToolCalls = accToolCalls.length > 0 && accToolCalls.some((t) => t.id && t.name);
          if (hasToolCalls) {
            const toolCalls = accToolCalls.filter((t) => t.id && t.name).map((t) => ({ id: t.id, type: "function" as const, function: { name: t.name, arguments: t.arguments } }));
            messages.push({ role: "assistant", content: streamedContent.trim() || null, tool_calls: toolCalls });
            ...
            if (finalContent) streamLine(encoder, controller, { type: "chunk", content: finalContent });
            const fullReply = [streamedContent.trim(), finalContent].filter(Boolean).join("\n\n") || FALLBACK_REPLY;
            if (conversationId) {
              await supabase.from("ai_messages").insert({ conversation_id: conversationId, role: "assistant", contenu: fullReply });
              await supabase.from("ai_conversations").update({ date_mise_a_jour: new Date().toISOString() }).eq("id", conversationId);
            }
            streamLine(encoder, controller, { type: "done", content: fullReply, conversationId: conversationId ?? undefined });
          } else {
