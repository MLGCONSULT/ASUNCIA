...
function getPendingFallbackKey(
  provider: PendingOAuthProvider,
  state: string
): string {
...
export function getProviderRuntimeMode(
...
async function cleanupExpiredPendingStates(): Promise<void> {
  ...
  if (error && !isMissingTableError(error)) {
    throw new Error(
      `Impossible de nettoyer les etats OAuth expires: ${error.message}`
    );
  }
}

export async function persistPendingOAuthState(
  input: PersistPendingOAuthInput
): Promise<void> {
  const supabase = createSupabaseServiceClient();
  const expiresAt = new Date(
    Date.now() + (input.ttlMs ?? DEFAULT_PENDING_TTL_MS)
  ).toISOString();
  ...
  if (error) {
    ...
    throw new Error(
      `Impossible d'enregistrer l'etat OAuth ${input.provider}: ${error.message}`
    );
  }
}
...
export async function consumePendingOAuthState(
  ...
  const { data, error } = await supabase
    .from("oauth_pending")
    .select(
      "id, utilisateur_id, code_verifier, redirect_uri, client_id, client_secret, expires_at"
    )
  ...
  if (error) {
    ...
    throw new Error(
      `Impossible de lire l'etat OAuth ${provider}: ${error.message}`
    );
  }
...
export async function getStoredOAuthClient(
  ...
  if (error) {
    ...
    throw new Error(
      `Impossible de lire le client OAuth ${provider}: ${error.message}`
    );
  }
...
export async function upsertStoredOAuthClient(
  input: StoredOAuthClientInput
): Promise<void> {
  const supabase = createSupabaseServiceClient();
  ...
  if (error) {
    ...
    throw new Error(
      `Impossible d'enregistrer le client OAuth ${input.provider}: ${error.message}`
    );
  }
}
