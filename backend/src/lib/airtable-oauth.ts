/**
 * OAuth 2.0 + PKCE pour le MCP Airtable officiel (mcp.airtable.com).
 * Voir https://support.airtable.com/docs/using-the-airtable-mcp-server
 */
...
/**
 * Construit l'URL d'autorisation pour rediriger l'utilisateur vers Airtable.
 */
...
export async function exchangeAirtableCodeForTokens(
  ...
  // Si client_secret est fourni, l'ajouter dans le body (certaines implémentations le requièrent)
  if (clientSecret) {
...
export async function getValidAirtableAccessToken(
  ...
  // Si le token est encore valide (avec marge de 60 secondes), le retourner
