...
const NAV_ITEMS: { href: string; label: string; icon: IconName }[] = [
  { href: "/app/dashboard", label: "Assistant IA", icon: "chat" },
  ...
  return (
    <>
      <header className="sticky top-0 z-50 glass-strong border-b border-white/10">
        <div className="flex items-center justify-between max-w-6xl mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <Link
            href="/app/dashboard"
            className="flex items-center gap-2 text-lg sm:text-xl font-bold font-display text-text-primary hover:text-accent-cyan transition-colors duration-200"
          >
          ...
          <nav className="hidden md:flex items-center gap-1">
              ...
              return (
                <Link
                  ...
                  className={`relative px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                    isActive ? "text-accent-cyan" : "text-text-muted hover:text-text-primary hover:bg-white/5"
                  }`}
                >
                  {isActive && (
                    <motion.span
                      layoutId="nav-pill"
                      className="absolute inset-0 rounded-lg bg-accent-cyan/10 border border-accent-cyan/30"
            ...
            <form onSubmit={handleLogout} className="ml-2">
              <button
                type="submit"
                className="px-3 py-2 rounded-lg text-sm text-text-muted hover:text-accent-rose hover:bg-accent-rose/5 transition-colors duration-200"
              >
                Déconnexion
              </button>
            </form>
          ...
          <div className="flex items-center gap-2 md:hidden">
            ...
            <motion.button
              type="button"
              aria-label="Menu"
              className="p-2 rounded-lg text-text-primary hover:bg-white/10"
                    ...
                    >
                      <Link
                        ...
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${
                          isActive ? "bg-accent-cyan/15 text-accent-cyan border border-accent-cyan/30" : "text-text-muted hover:bg-white/5 hover:text-text-primary"
                        }`}
                      >
