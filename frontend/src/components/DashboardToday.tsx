...
export default function DashboardToday() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [gmailStatusRes, gmailUnreadRes, n8nRes, airtableRes] = await Promise.all([
          fetchBackend("/api/gmail/status").catch(() => null),
          fetchBackend("/api/gmail/messages?maxResults=20&q=is:unread").catch(() => null),
          fetchBackend("/api/n8n/workflows?limit=50").catch(() => null),
          fetchBackend("/api/airtable/bases").catch(() => null),
        ]);

        const gmailStatus = gmailStatusRes?.ok ? await gmailStatusRes.json().catch(() => ({})) : {};
        const gmailUnreadData = gmailUnreadRes?.ok ? await gmailUnreadRes.json().catch(() => ({})) : {};
        const n8nData = n8nRes?.ok ? await n8nRes.json().catch(() => ({})) : {};
        const airtableData = airtableRes?.ok ? await airtableRes.json().catch(() => ({})) : {};

        const unreadMessages = Array.isArray(gmailUnreadData?.messages) ? gmailUnreadData.messages : [];
        ...
        if (!active) return;
        setState({
          gmailUnread: unreadMessages.length,
  ...
  const quickPrompt = useMemo(
    () =>
      buildAssistantPromptUrl(
        "Fais-moi un plan d'action du jour en 5 points a partir de mes mails, workflows et bases connectees.",
      ),
    [],
  );
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-3 gap-3">
      <div className="glass-strong rounded-xl border border-white/10 p-4 card-glow">
        <p className="text-xs uppercase tracking-[0.18em] text-text-dim">Aujourd'hui - Mails</p>
        {state.loading ? (
          <p className="mt-2 text-sm text-text-muted">Chargement...</p>
        ) : !state.gmailConnected ? (
          <>
            <p className="mt-2 text-sm text-text-muted">Gmail non connecte.</p>
            <Link href="/app/mails" className="inline-block mt-3 text-xs text-accent-cyan hover:underline">
              Connecter Gmail
            </Link>
          </>
        ) : (
          <>
            <p className="mt-2 text-2xl font-semibold text-text-primary">{state.gmailUnread ?? 0}</p>
            <p className="text-xs text-text-muted mt-1">emails non lus a traiter</p>
            <div className="mt-3 flex gap-2">
              <Link href="/app/mails" className="text-xs text-accent-cyan hover:underline">
                Ouvrir Mails
              </Link>
              <Link
                href={buildAssistantPromptUrl("Resume mes emails non lus et priorise mes reponses.")}
      ...
      <div className="lg:col-span-3 rounded-xl border border-accent-violet/25 bg-accent-violet/10 p-3">
